...
# 用法： bash ppt-gen/build.sh [脚本] [产物名]
# 例：   bash ppt/build.sh ppt/build_deck.py ATRI-答辩PPT-v5
set -euo pipefail
...
SCRIPT="${1:-ppt-gen/build_deck.py}"
...
export_pdf() {
  ...
  if command -v osascript >/dev/null 2>&1 && [[ -f "$ROOT/ppt-gen/export_pdf.applescript" ]]; then
    osascript -e 'tell application "Microsoft PowerPoint" to close every presentation saving no' \
      >/dev/null 2>&1 || true
    sleep 1
    rm -f "$ROOT/ppt/out/~\$"*.pptx
    osascript "$ROOT/ppt-gen/export_pdf.applescript" "$PPTX" "$PDF"
    return 0
  fi
