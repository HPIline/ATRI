...
# 深色图版页专用。取值直接取样自 design/renders 系列 CAD 渲染图的底色：
# 页面底色与图片底色差在 1 个色阶以内，贴图时看不出矩形接缝。
DARK = "323A42"       # 图版页底色（= ATRI-v2 渲染图底色）
DARK_TEXT = "0A2540"  # 图版页左侧文字栏底色（墨蓝，承接章节页语言）
DARK_2 = "3C454D"     # 图版页网格线
DARK_TXT = "C9D3DC"   # 图版页次要文字
DARK_LINE = "4A535B"  # 图版页分隔线
...
def num_card(slide, x, y, w, h, number, unit, caption, color=INK, unit_size=15,
    ...
    数值行与说明行的偏移按字号推导（行高 ≈ 1.45 em），不写死 0.66 / 0.84 英寸：
    写死的偏移在字号变化时会让说明行压到数值行上，也会让版面检查误判越界。
    行高 1.45 em 与 textmetrics.LINE_FACTOR 保持一致。
    """
    rect(slide, x, y, w, h, fill=WHITE, line=LINE, line_w=0.75)
    rect(slide, x, y, 0.045, h, fill=BLUE)
    val_h = num_size * 1.45 / 72.0
    text(slide, x + 0.24, y + 0.14, w - 0.40, val_h,
         [P([R(number, num_size, color, heavy=True),
             R(" " + unit, unit_size, color, heavy=True)], line_spacing=1.0)])
    cap_y = y + 0.14 + val_h + 0.08
    text(slide, x + 0.24, cap_y, w - 0.40, max(0.20, y + h - cap_y - 0.08),
...
def picture_fit(slide, x, y, w, h, path):
    """把图片整体放进 x,y,w,h（等价 CSS object-fit: contain），居中留白。

    与 picture_cover 的区别：cover 会裁掉超出部分，用在需要"填充画框"的场面图；
    fit 不裁，用在**工程图与三维视图**上——裁掉一脚或一头，图纸就不成立了。
    """
    from PIL import Image
    ...
    if img_ar > box_ar:
        ...
        nh, nw = h, h * img_ar
    return slide.shapes.add_picture(path, Inches(x + (w - nw) / 2),
                                    Inches(y + (h - nh) / 2),
                                    Inches(nw), Inches(nh))
