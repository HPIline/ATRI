    ...
    for idx, slide in enumerate(prs.slides, 1):
        ...
        for shape in slide.shapes:
            if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                ...
                iw, ih = im.size
                box = (int(round(shape.crop_left * iw)),
                       int(round(shape.crop_top * ih)),
                       int(round((1 - shape.crop_right) * iw)),
                       int(round((1 - shape.crop_bottom) * ih)))
                im = im.crop(box).resize((max(1, w), max(1, h)), Image.LANCZOS)
                img.paste(im, (x, y), im)
                continue
