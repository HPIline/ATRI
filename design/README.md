...
>   结构件 **1790 g**（真实体积实算）、整机 **3.437 kg**、**实装包络 418 × 223 × 129 mm**。
>   力矩因此超标（踝 111%），减重路径 A/B/C 见 `handoff/新架构参数总表.md` §3.1。
...
├── gen_v3_baseline.py        # v2→v3 一次性迁移：并入 CAD 实装包络与结构件实算质量
├── reference/cad_assembly_measurements.json  # CAD 实装测量（包络/零件质量/减重路径）
