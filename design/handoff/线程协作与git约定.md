...
> ⛔ **2026-09-12 作废**：下表点名的若干路由**本机不存在**（例如 `kimi-k2.7-code`，
> 实际注册的是 `kimi-k3`），招募时无法按名启动；现行方案只保留两条本机路由
> （组长 + 按需升级的重推理）。**本机路由细节故意不入库**，见本机文件
> `design/handoff/本机线程路由.md`（由 `.git/info/exclude` 排除）；
> **项目级的文件领地纪律**见 `分工重整-两模型制.md` 第 4 节。
> 本表仅作历史留档，**不要再照此招募线程**。
