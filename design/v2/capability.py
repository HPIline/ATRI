"""Capability evidence for the new mechanism; legacy software is not a robot test."""
...
def matrix() -> List[Dict[str,str]]:
    ...
    return [dict(zip(('id','name','verdict','gates','evidence'),row)) for row in data]


def markdown():
    lines=['# ATRI-v2能力验证审查','','没有真机日志的项目不写成实测通过。','',
           '| ID | 项目 | 判定 | 门禁 | 依据 |','|---|---|---|---|---|']
    for row in matrix():lines.append('| '+' | '.join(row[k] for k in ('id','name','verdict','gates','evidence'))+' |')
    lines+=['','G4之前不得修改现机22DOF软件或恢复hip_yaw。所有结构与采购更改继续受A路线预算、质量和门禁约束。','']
    return '\n'.join(lines)
