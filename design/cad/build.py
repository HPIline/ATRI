...
1. ~~补齐 `STS3215.body_mount_spacing_mm`~~ → **已闭环（2026-09-11）**：
   该字段描述的"安装耳"**不存在**，已删除；真实接口为两端面 Φ20 + 4×Φ2.5 @ Φ14。
   见 `design/handoff/STS3215-机械接口核验.md`
2. ~~复核舵盘节圆（白皮书给了 Φ14 与 Φ16 两个值）~~ → **已闭环**：Φ14 经第二个独立来源确认
3. 按同一套 `standards.py` 逐个建立 22 个关节的模块与连杆
4. 组装成装配体，做**干涉检查**与**装配顺序**验证
5. **待实物复测的三项**（买 1 只 STS3215，约 90 元）：
   - 输出轴相对机身基准面的偏心量（模型坐标系与实物 A/B/C 尚未对应）
   - Φ2.5 孔的螺纹规格（M2.5 螺纹孔 or M2/M3 底孔）
   - 副轴轴径与伸出长度（模型只见 0.6 mm 凸台，与现有值 3.0 冲突 5 倍）
"""
    path = OUT / "report.md"
