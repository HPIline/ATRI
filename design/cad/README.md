...
**现行 CAD 口径（2026-09-12 第 7 轮，`atri-next`）**：整机包络 **407 × 263 × 147 mm**（高×宽×深；足长 122 把第 6 轮的 149 收回）、
宽上限门禁 270 mm（赛框 300）。布局门禁见 `design/cad/test_layout.py`。
结构件质量方案仍未定案；踝 1.492 N·m / 腰 1.899 N·m 口径未改。
第 6 轮改的是布置（U 形骨盆、头壳让位、肘/爪零位、足后跟），不是 AABB 切口。
