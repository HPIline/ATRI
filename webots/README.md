...
> **默认世界是 20 DOF v2 运动学联调世界** `worlds/atri_v2.wbt`（无 hip_yaw，控制器 `atri_controller`）。冻结的现机 22 DOF 世界仍在 `worlds/atri_22dof.wbt`，不要当 v2 证据。CAD 网格导入冒烟见 `design/v2/WEBOTS-STATUS.md`。默认批量脚本跑的是 **零重力关节下发/回读**，不是 G4，不是真机。
