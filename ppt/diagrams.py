...
# 成对关节：(编号, |x|, y, 组)
_PAIRS = [
    (14, 11.0, 30, "arm"), (15, 13.0, 34, "arm"),
    (16, 14.0, 50, "arm"), (17, 15.0, 64, "arm"),
    (4, 7.0, 44, "leg"), (5, 8.0, 48, "leg"), (6, 8.5, 52, "leg"),
    (7, 9.0, 74, "leg"), (8, 9.5, 93, "leg"),
]
...
def joint_topology(slide, cx, top, h, labels=True, dim=False):
    ...
    # ── 四肢连杆
    for g in (-1, 1):
        link(g * 11, 30, g * 13, 34)         # 上臂
        link(g * 13, 34, g * 14, 50)         # 前臂
        link(g * 14, 50, g * 15, 64)         # 末端
        link(g * 7, 44, g * 8, 48)           # 髋
        link(g * 8, 48, g * 8.5, 52)         # 大腿根
        link(g * 8.5, 52, g * 9, 74)         # 大腿
        link(g * 9, 74, g * 9.5, 93)         # 小腿
    ...
    for jid, y, side, grp in _SPINE:
        ...
        if labels:
            label(jid, 0, y, side)
    for jid, ax, y, grp in _PAIRS:
        for g in (-1, 1):
            joint(g * ax, y, grp)
            if labels:
                label(jid, g * ax, y, "l" if g < 0 else "r")
...
def joint_topology_legend(slide, x, y, w):
    """结构图图例：四组自由度的颜色/数量说明。"""
    rows = [
        ("双腿 10", "每腿 髋 3（yaw/roll/pitch）· 膝 1 · 踝 1"),
        ("双臂 8", "每臂 肩 2（pitch/roll）· 肘 1 · 夹爪 1"),
...
def chapter_page(slide, num, title, sub=None):
    ...
    for i in range(22):
        ...
        if i < 21:
            _seg(slide, 11.90, y + 0.05, 11.90, y + 0.20, "2E5A82", 1.0)
