...
def build_report(det_rows: List[DetectorRow], det_sum: Dict[str, Any],
    ...
    add("")
    add("**先分清两个 gain**（混起来会把参数调反）：")
    add("")
    add("- `gain_cm_per_deg`（几何量）= 身体转 1°，目标在画面里横移几厘米 = **物距 × tan(1°)**；")
    add("- `step_gain`（技能参数）= 看到 1 cm 偏差就转几度。理想死拍值是 `1 / gain`，")
    add("  但模型不准时会震荡，工程上取其 1/3–1/2，并用 §4 的边界表复核迭代上限。")
    add("")
    ...
    add("## 6. 与实物对接：改哪里、量什么")
    add("")
    add("1. **相机标定**后分两处落值（**这两个量不是一回事**）：")
    add("   - 检测器换算：像素当量 `ppcm = f / 物距`（`OpenCVPerception(pixels_per_cm=…)`；")
    add("     本工具用 `--pixels-per-cm` 传）；")
    add("   - 技能参数：`step_gain` = 每 cm 偏差转几度。理想死拍值 = `1 / (0.01745 × 物距cm)`")
    add("     （12 cm 物距 ≈ 4.8）；越接近死拍值越快、越怕模型不准，故先取 1/5–1/2")
    add("     （§4.1 用的 1.0 ≈ 死拍值的 1/5），再用 §4 的边界表复核 `max_iters`；")
    add("   - `grasp_tolerance_cm`（夹爪能容忍多大偏差，实机标定）与 `deadband_cm`（收敛目标）。")
    add("2. **位移标定**后写 `config/odometry.json` 与 `step_length_cm`（现在都是名义值）。")
