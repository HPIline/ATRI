...
- **体育运动-踢球**（T-04）：绿球色块 + 多轮偏航闭环后侧踢；不收敛也尽力踢，但必须回报 `iterations/converged`；
  **缺横向偏移不再当成"正前方"**（判失败）。参数可标定（`config/kick.json`），检测与闭环有可复跑数据（**合成图/一维几何口径，非实机**），见下方 T-04 小节。
- **娱乐休闲-舞蹈**（T-05）：离线关键词触发 22 轴短舞 + TTS；可选本地 wav（无播放器标 `audio=unavailable`）。
  **vosk 真引擎已接入并在独立 venv 跑通真解码**（合成语音口径），结果里 `asr_engine`/`asr_real` 明写这次是不是真识别，见下方 T-05 小节。
...
# 4. 运行 Webots 控制器及映射离线自检（切回仓库根目录，41 项全部通过）
cd ../..
