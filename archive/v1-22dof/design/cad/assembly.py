...
BACK_X = 60.5
#
# ⚠️ 2026-09-12「板子怎么固定在机体上」——这条接口以前没人写，结果整簇背挂悬空
#    （体检连通分量 2 个：板 + XL4015 + 喇叭自成一簇，板内侧到躯干框后表面实测 3.00 mm）。
#    接口的唯一真值在 skeleton.py，本表只负责"电子件贴板的哪一面/哪一点"：
#      背挂电子件 → 板**外**表面（本表，留 0.5 mm 气隙）
#      板 **内**侧 2 只安装耳（`skeleton.backpack_plate`）→ 压进后立柱后表面 1.5 mm
#        每只耳 2×M3 间隙孔（y=±48、z=6/12），与 `skeleton.torso_frame` 的 M3 自攻底孔同轴；
#        立柱 z ≥ 16 才有 Ø5 走线孔 ⇒ 落点全在实心段，单颗螺钉 8 mm 啮合。
#    即载荷路径：电子件 → 背板 → 安装耳 → 躯干框后立柱 → 双肩 pylon。
BACKPACK_PITCH = 16.0
