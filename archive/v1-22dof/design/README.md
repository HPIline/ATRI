...
> **当前 A 路线机构在 [`design/v2/`](v2/README.md)。** 那是 20 DOF、无 hip_yaw、6061 铝夹层 + 2.4 mm PETG、单电池、单舵机两指夹爪的实装审查设计。本文件其余部分描述的是**旧 22 DOF 设计数据包与仿真链路**——`robot_model.json`、`atri.urdf`、`cad/` 属冻结的现机 22 DOF / 上一代 CAD 线，**不是 v2 机构**。下文的 "22 关节"、"L1 是唯一来源"等表述仅对这条冻结的旧线成立，不要读成 v2。
