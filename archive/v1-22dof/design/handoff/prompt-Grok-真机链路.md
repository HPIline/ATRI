...
1. **`software/atri/atri/bus_sts3215.py`**：实现 `atri.cerebellum.ServoBus` 的全部语义
...
2. **`software/atri/tests/test_bus_sts3215.py`**：用**假串口**（`io.BytesIO` / mock 对象）验证
...
- **只碰这些文件**：`software/atri/atri/bus_sts3215.py`、`software/atri/tests/test_bus_sts3215.py`、
  `固件/**`、`design/handoff/上下位机协议.md`、`design/handoff/协议校核-*.md`。
  `atri/config.py` / `cerebellum.py` 已冻结语义，需要新方法就在报告里**提议 patch**，不要直接改。
- **不要碰**：`design/**`、`webots/**`、`ppt/**`、`software/atri/atri/skills/**`、`task_cards/**`。
- **不要手改生成物**：`design/placements.json`、`design/cad/out/**`。
- 主包核心保持**零第三方依赖**；串口依赖只加在 `software/atri/requirements.txt` 且惰性导入。
- 中文回复；**报告必须给命令 + 输出**，不要只说"已完成"；找不到就写"未找到"，**禁止编造寄存器地址**。
- 每完成一项先跑测试：`cd software/atri && python3 -m unittest discover -s tests`（现有 **186 项必须全绿**）
